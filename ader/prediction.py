"""
Prediction conversion utilities for ADER.

This module converts model logits into entity and relation predictions and
stores prediction results in JSON format.
"""
import json
...
def convert_predictions(batch_entity_clf: torch.tensor, batch_rel_clf: torch.tensor,
    ...
    batch_rel_clf = batch_rel_clf.clone()
...
def _convert_pred_relations(rel_clf: torch.tensor, rels: torch.tensor,
    ...
    if valid_rels.shape[0] != 0:
        pred_rel_entity_types = entity_types[valid_rels]
...
def store_predictions(documents, pred_entities, pred_relations, store_path):
    ...
    # store as json
    with open(store_path, "w", encoding="utf-8") as predictions_file:
        json.dump(predictions, predictions_file, ensure_ascii=False, indent=2)
